...
def generate_face_faceDir(input_root, save_root, savetype="image"):
    if savetype == "image":
        for dir_path in sorted(
            glob.glob(input_root + "/*_aligned")
        ):  # 'xx/xx/000100_guest_aligned'
            frame_names = os.listdir(dir_path)  # ['xxx.bmp']
            if len(frame_names) != 1:
                continue
            frame_path = os.path.join(
                dir_path, frame_names[0]
            )  # 'xx/xx/000100_guest_aligned/xxx.bmp'
...
def extract(
    input_dir, process_type, save_dir, face_dir, hog_dir, pose_dir, name_npy=None
):
    ...
    for i, vid in enumerate(vids, 1):
        ...
        if saveVid not in process_names:
            continue
        ...
        if not os.path.exists(hog_root):
            os.makedirs(hog_root)
        if not os.path.exists(pose_root):
            os.makedirs(pose_root)
        if process_type == "faceDir":
            ...
            generate_face_faceDir(
                save_root, face_root, savetype="npy"
            )  # compress frames into npy
            ...
            exe_path = os.path.join(
                config.PATH_TO_OPENFACE_Win, "build/bin/FeatureExtraction"
            )  # jxie
...
if __name__ == "__main__":
    ...
    input_dir = os.path.join(dataset, "video")

    # out: feature csv dir
    save_dir = os.path.join(dataset, "features", "openface_all")
    hog_dir = os.path.join(dataset, "features", "openface_hog")
    pose_dir = os.path.join(dataset, "features", "openface_pose")
    face_dir = os.path.join(dataset, "features", "openface_face")
    ...
    # process
    extract(
        input_dir, process_type, save_dir, face_dir, hog_dir, pose_dir, params.name_npy
    )
