...
def read_hog(filename, batch_size=5000):
    ...
    with open(filename, "rb") as f:
        ...
        feature_vector = struct.unpack(
            "{}f".format(num_features), f.read(num_features * 4)
        )
        ...
        while True:
            ...
            assert (
                num_bytes_read % 4 == 0
            ), "Number of bytes read does not match with float size"
            num_floats_read = num_bytes_read // 4
            assert (
                num_floats_read % num_floats_per_feature_vector == 0
            ), "Number of bytes read does not match with feature vector size"
